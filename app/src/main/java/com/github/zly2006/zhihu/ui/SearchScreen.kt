            ...
            TopAppBar(
                ...
                actions = {
                    IconButton(
                        ...
                        enabled = viewModel.searchQuery.isNotEmpty(),
    ...
    DropdownMenu(
        ...
        SearchSortOption.entries.forEach { option ->
            SearchFilterMenuItem(
                ...
                onClick = {
                    onDismissRequest()
                    viewModel.updateSortOption(context, option)
                },
            )
        }
        HorizontalDivider()
        ...
        SearchContentType.entries.forEach { type ->
            SearchFilterMenuItem(
                ...
                onClick = {
                    onDismissRequest()
                    viewModel.updateContentType(context, type)
                },
            )
        }
        HorizontalDivider()
        ...
        SearchTimeRange.entries.forEach { range ->
            SearchFilterMenuItem(
                ...
                onClick = {
                    onDismissRequest()
                    viewModel.updateTimeRange(context, range)
                },
            )
        }
    }
